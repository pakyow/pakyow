  ...
  context "view path exists" do
    ...
    it "presents the object in the form" do
      ...
      expect(response[2]).to include_sans_whitespace(
        <<~HTML
          <form data-b="post" data-ui="form" data-c="form" class="" action="/posts/2" method="post" data-id="2">
        HTML
      )
      ...
      expect(response[2]).to include_sans_whitespace(
        <<~HTML
          <input type="text" data-b="title" name="post[title]" class="" title="" value="bar">
        HTML
      )
    end
  end
