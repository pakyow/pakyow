RSpec.describe "single form endpoint: nested resource list" do
  ...
  it "defines a route for the view" do
    expect(controller(:posts, :comments).routes[:get][0].name).to eq(:list)
    expect(controller(:posts, :comments).routes[:get][0].path).to eq("/")
  end
  ...
  describe "behavior" do
    ...
    it "sets up the form for creating" do
      expect(call("/posts/1/comments")[2]).to include_sans_whitespace(
        <<~HTML
