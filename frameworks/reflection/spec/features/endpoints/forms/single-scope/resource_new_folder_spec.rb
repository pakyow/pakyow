RSpec.describe "single form endpoint: resource new folder" do
  include_context "reflectable app"
  include_context "mirror"

  let :frontend_test_case do
    "endpoints/forms/single-scope/resource-new-folder"
  end
  ...
  it "defines a route for the view" do
    expect(controller(:posts).routes[:get][0].name).to eq(:new)
    expect(controller(:posts).routes[:get][0].path).to eq("/new")
  end
