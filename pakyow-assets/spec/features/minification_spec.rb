RSpec.describe "minifying assets" do
  ...
  context "app is configured to minify" do
    ...
    it "minifies sass" do
      expect(call("/assets/types-sass.css")[2].body.read).to eq("body{background:#f33}\n")
    end

    it "minifies scss" do
      expect(call("/assets/types-scss.css")[2].body.read).to eq("body{background:#f33}\n")
    end

    it "minifies js" do
      expect(call("/assets/default.js")[2].body.read).to eq("\"use strict\";console.log(\"foo\"),console.log(\"bar\");")
    end

    it "minifies external js" do
      expect(call("/assets/packs/external-transpiled.js")[2].body.read).to eq_sans_whitespace(
        <<~MINIFIED
  ...
  context "app is not configured to minify" do
    ...
    it "does not minify sass" do
      expect(call("/assets/types-sass.css")[2].body.read).to eq("body {\n  background: #ff3333; }\n")
    end
    ...
    it "does not minify js" do
      expect(call("/assets/default.js")[2].body.read).to eq("\"use strict\";\n\nconsole.log(\"foo\");\nconsole.log(\"bar\");")
    end
  end
end
